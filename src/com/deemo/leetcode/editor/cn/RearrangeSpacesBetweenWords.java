...
public class RearrangeSpacesBetweenWords{
    public static void main(String[] args) {
        ...
        System.out.println(solution.reorderSpaces("   world"));
    ...
    class Solution {
        public String reorderSpaces(String text) {
            ...
            for (String string : strings) {
                cntSpace -= string.length();
            }

            StringBuilder sb = new StringBuilder(text.length());
            int i, n = strings.length;
            int per = n == 1 ? cntSpace : cntSpace / (n - 1);
            StringBuilder space = new StringBuilder(per);
            while (space.length() < per) {
                space.append(' ');
            }
            for (i = 0; i < n - 1; i++) {
                sb.append(strings[i]).append(space);
            }
            sb.append(strings[i]);
